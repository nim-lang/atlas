...
suite "test expand with git tags":
  ...
  test "expand from file":
      ...
      withDir "tests/ws_testtraverse":
        ...
        context().flags = {KeepWorkspace, ListVersions}
        context().defaultAlgo = SemVer
        discard context().nameOverrides.addPattern("$+", "file://." & DirSep & "buildGraph" & DirSep & "$#")
...
suite "test expand with no git tags":
  ...
  test "collect nimbles":
      ...
      withDir "tests/ws_testtraverse":
        ...
        context().flags = {KeepWorkspace, ListVersions}
        context().defaultAlgo = SemVer
        discard context().nameOverrides.addPattern("$+", "file://." & DirSep & "buildGraphNoGitTags" & DirSep & "$#")
  ...
  test "expand no git tags":
      ...
      withDir "tests/ws_testtraverse":
        ...
        var nc = createNimbleContext()
        discard nc.nameOverrides.addPattern("$+", "file://." & DirSep & "buildGraphNoGitTags" & DirSep & "$#")
